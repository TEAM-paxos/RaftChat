...
impl PersistentState {
    ...
    fn save_state(&self, voted_for: Option<&str>) -> bool {
    ...
    // If the new term is outdated, it does not update the term and returns 'false'.
    pub fn update_term(&mut self, new_term: u64) -> (u64, bool) {
        if new_term < self.current_term {
            ...
            let success = self.save_state(None);
            (self.current_term, success)
        }
    }

    // Attempts to vote for a candidate.
    // If no vote has been cast in the current term, it votes for the candidate and returns 'true'.
    pub fn try_vote(&mut self, candidate: &'static str) -> bool {
        match &self.voted_for {
            None => {
                ...
                // Save the updated state
                self.save_state(Some(candidate))
            }
